...
public class totem : MonoBehaviour {
	...
	public Transform piedra_transform;
	...
	void Start () {
		piedra_transform=GameObject.FindGameObjectWithTag("Piedra");
		text = GameObject.FindGameObjectWithTag ("mensaje");
	...
	void Update () {
		...
		distancia =Vector3.Distance(piedra_transform.position, this.transform.position);
		destroy ();
	}
	void  OnMouseEnter(){
		if (ishover == false) {
	...
	void destroy(){
		...
		if(distancia<1){
			text.GetComponent<Text>().text="Has perdido";
