...
public class WaveMesh : MonoBehaviour {
	...
	void Update () {
		...
		for (var i=0;i<vertices.Length;i++)
		{
			var vertex = verts[i];
			for (var j = 0; j < totem.Length; j++) {
				//Debug.Log (totem[j].transform.localScale.x);
				var scale = 100;
				tx = totem[j].transform.position.x/scale ;
				tz = totem[j].transform.position.z/-scale ;
				longitud = totem [j].GetComponent<totem> ().longitud;
				amplitud = totem [j].GetComponent<totem> ().amplitud/scale;
				velocidad = totem [j].GetComponent<totem> ().velocidad;
				damp = totem [j].GetComponent<totem> ().damp;
				ds = totem [j].GetComponent<totem> ().ds ;

				dist = Mathf.Sqrt (Mathf.Pow ( (tx - vertex.x)*longitud, 2) 
					+ Mathf.Pow ( tz - vertex.y , 2)*longitud)*scale;

				vertex.z += Mathf.Sin (ds + dist) * amplitud;//* damp;// (amplitud-Mathf.Clamp(amplitud*damp*dist,-amplitud,amplitud));
				}
